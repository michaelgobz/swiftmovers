...
from ..zones import ZoneContext, ZoneQsContext
from ..zones.utils.utils import get_default_zone_slug_or_graphql_error
from ..core.enums import OrderDirection
from ..core.types import NonNullList
from ..core.utils.sorting import sort_queryset_for_connection
...
def create_connection_slice(
    ...
    if isinstance(iterable, ZoneQsContext):
    ...
    if isinstance(iterable, ZoneQsContext):
        ...
        for edge in slice.edges:
            node = edge.node
            edge.node = ZoneQsContext(node=node, zone_slug=iterable.zone_slug)
...
def filter_connection_queryset(iterable, args, request=None, root=None):
    ...
    if filter_input:
        ...
        except (NoDefaultZone, ZoneNotDefined, GraphQLError, KeyError):
            filter_channel = None
        filter_input["channel"] = (
            args.get("channel")
            or filter_channel
            or get_default_zone_slug_or_graphql_error()
        )
        ...
        if isinstance(iterable, ZoneQsContext):
            return ZoneQsContext(filterset.qs, iterable.zone_slug)
