...
def test_calculate_checkout_total(
    ...
    monkeypatch.setattr(
        "swiftmovers.plugins.avatax.plugin.AvataxPlugin._skip_plugin", lambda *_: False
    )
    manager = get_plugins_manager()
    checkout_with_item.shipping_address = ship_to_pl_address
    checkout_with_item.save()
...
def test_get_cached_tax_codes_or_fetch(monkeypatch, avatax_config):
    monkeypatch.setattr("swiftmovers.plugins.avatax.cache.get", lambda x, y: {})
...
def test_get_checkout_line_tax_rate_error_in_response(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "swiftmovers.plugins.avatax.plugin.get_checkout_tax_data",
...
def test_get_order_line_tax_rate_error_in_response(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "swiftmovers.plugins.avatax.plugin.get_order_tax_data",
        lambda *_: {"error": "Example error"},
    )
    unit_price = TaxedMoney(Money(12, "USD"), Money(15, "USD"))
...
def test_get_checkout_shipping_tax_rate_skip_plugin(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "swiftmovers.plugins.avatax.plugin.AvataxPlugin._skip_plugin",
        lambda *_: True,
    )
    shipping_price = TaxedMoney(Money(12, "USD"), Money(15, "USD"))
...
def test_get_plugin_configuration(settings, channel_USD):
    settings.PLUGINS = ["swiftmovers.plugins.avatax.plugin.AvataxPlugin"]
    manager = get_plugins_manager()
...
@patch("swiftmovers.plugins.avatax.plugin.api_get_request")
...
@patch("swiftmovers.plugins.avatax.plugin.api_post_request_task.delay")
@override_settings(PLUGINS=["swiftmovers.plugins.avatax.plugin.AvataxPlugin"])
...
def test_api_get_request_handles_request_errors(product, monkeypatch, avatax_config):
    ...
    monkeypatch.setattr("swiftmovers.plugins.avatax.requests.get", mocked_response)
...
def test_api_post_request_handles_request_errors(product, monkeypatch, avatax_config):
    mocked_response = Mock(side_effect=RequestException())
    monkeypatch.setattr("swiftmovers.plugins.avatax.requests.post", mocked_response)
...
@patch("swiftmovers.plugins.avatax.get_order_request_data")
@patch("swiftmovers.plugins.avatax.get_cached_response_or_fetch")
...
@patch("swiftmovers.plugins.avatax.plugin.get_checkout_tax_data")
