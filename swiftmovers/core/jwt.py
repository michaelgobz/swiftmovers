...
JWT_swiftmovers_OWNER_NAME = "swiftmovers"
...
def jwt_user_payload(
    ...
    token_owner: str = JWT_swiftmovers_OWNER_NAME,
) -> Dict[str, Any]:
...
def create_token(payload: Dict[str, Any], exp_delta: timedelta) -> str:
    payload.update(jwt_base_payload(exp_delta, token_owner=JWT_swiftmovers_OWNER_NAME))
...
def is_swiftmovers_token(token: str) -> bool:
    """Confirm that token was generated by swiftmovers not by plugin."""
    try:
        payload = jwt.decode(token, options={"verify_signature": False})
    except jwt.PyJWTError:
        return False
    owner = payload.get(JWT_OWNER_FIELD)
    if not owner or owner != JWT_swiftmovers_OWNER_NAME:
        return False
    return True
...
def create_access_token_for_app(app: "App", user: "User"):
    ...
    App can use user's JWT token to proceed given operation in swiftmovers.
