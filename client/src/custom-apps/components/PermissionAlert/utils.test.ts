...
describe("Permission Parsing", () => {
  it("should extract permissions from the meta `description` if available", () => {
    // Arrange
    // -> Order.invoices
    //    https://docs.swiftmovers.io/docs/3.x/api-reference/objects/order
    const description = `List of order invoices. Can be fetched for orders created in swiftmovers 3.2 and later, for other orders requires one of the following permissions: MANAGE_ORDERS, OWNER.`;
