...
Run `cp .env.template .env` to create default configuration that will be connected to [swiftmovers Demo shop](https://demo.swiftmovers.io/dashboard/)

## All environment variables

Create or edit `.env` file in a root directory or set environment variables with the following values:

- `API_URI` (required) - URI of swiftmovers GraphQL API instance.
  If you are running swiftmovers locally with the default settings, set `API_URI` to: "http://localhost:8000/graphql/".
...
- `STATIC_URL` - URL where the static files are located.
  E.g., if you use an S3 bucket, you should set it to the bucket's URL. By default, swiftmovers assumes you serve static files from the root of your site at "http://localhost:9000/".
